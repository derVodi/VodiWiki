...
Story.prototype.switchTheme = function(theme) { // todo: remove theming
	if (safeMode)
		return;
	...
	var getSlice = function(theme, slice) {
		...
		if (r && r.indexOf(config.textPrimitives.sectionSeparator) == 0) r = theme + r;
	...
	if (!startingUp) {
		if (config.refresherData.pageTemplate!=pt || config.tiddlerTemplates[vi] != vt || config.tiddlerTemplates[ei] != et) {
			...
			setStylesheet(store.getArticleRawTextResolveIncludes(config.refresherData.styleSheet,"",10),config.refreshers.styleSheet);
		}
